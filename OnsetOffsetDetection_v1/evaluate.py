...
DEFAULT_OUTPUT_DIR = REPO_ROOT / "OnsetOffsetDetection_v1" / "eval_outputs"


def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate OnsetOffsetDetection_v1 on GTSinger English.")
        ...
        with torch.inference_mode():
            ...
            for index, metadata in enumerate(batch["metadata"]):
                ...
                times = (
                    ...
                    * module.config_obj.features.frame_seconds
                )
...
def aggregate_section(rows: list[dict[str, Any]], section: str) -> dict[str, Any]:
    keys = ("precision", "recall", "f1", "mae_sec")
    output = {}
    for key in keys:

...
try:
    ...
    raise RuntimeError("lightning is required for VoiceCoachDataModule") from exc
...
class VoiceCoachDataModule(L.LightningDataModule):
    """LightningDataModule for runtime feature and label conversion.

    The DataLoader returns raw CPU batches. Call ``transfer_batch_to_device`` or
    use Lightning's automatic batch transfer to convert audio/features/labels on
    the target device without saving .npy files.
    """

    def __init__(self, config: VoiceCoachDataConfig):
        super().__init__()
        self.config = config
        self.raw_collator = VoiceCoachRawBatchCollator()
        self.device_converters: dict[str, VoiceCoachDeviceBatchConverter] = {}
    ...
    def transfer_batch_to_device(
        ...
        if key not in self.device_converters:
            self.device_converters[key] = VoiceCoachDeviceBatchConverter(

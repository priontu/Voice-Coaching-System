...
# Config (used as defaults when no CLI args are supplied)
...
def main():
    ...
    parser.add_argument(
        "--pitch-json", "-p",
        default=PITCH_JSON_PATH,
        help=f"Path to pitch_data.json produced by inference.py (default: {PITCH_JSON_PATH})",
    )
    parser.add_argument(
        "--musicxml", "-m",
        default=MUSICXML_PATH,
        help=f"Path to the MusicXML reference file (default: {MUSICXML_PATH})",
    )
    args = parser.parse_args()

    pitch_json_path = args.pitch_json

...
from OnsetOffsetDetection_v4.lightning_module import OnsetOffsetLightningModule, OnsetOffsetTrainingConfig
from OnsetOffsetDetection_v4.model import MambaModelConfig

DEFAULT_DATASET_ROOT = Path("/mnt/archive/GTSinger/English")
DEFAULT_OUTPUT_DIR = REPO_ROOT / "OnsetOffsetDetection_v4" / "runs" / "default"


def main() -> None:
    parser = argparse.ArgumentParser(description="Train OnsetOffsetDetection_v4 (Mamba backbone, no length cap).")
    ...
    parser.add_argument("--accelerator", default="gpu")   # mamba-ssm requires CUDA
    parser.add_argument("--devices", default="1")
    parser.add_argument("--precision", default="bf16-mixed")
    parser.add_argument("--patience", type=int, default=20,
                        help="Early stopping patience (epochs). 0 disables early stopping.")
    # max_audio_sec defaults to None in config (no cap needed with Mamba)
    ...
    training_config = OnsetOffsetTrainingConfig(
        ...
        max_audio_sec=None,  # no cap — Mamba is O(n)
    )

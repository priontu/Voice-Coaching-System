...
- `tests/` — 90+ pytest tests; run with `cd pipeline && python -m pytest tests/ -q`

### `hf_models/` — HuggingFace-based model wrappers (Priontu)
...
- Calls `pipeline/inference/run_pitch.py` and `pipeline/scoring/pitch_score.py` as subprocesses
- **Known issue:** `SONG_REFERENCE_DIR` in `frontend/app.py:27` points to a root-level `song_references/` that doesn't exist — actual reference files are in `pipeline/song_references/`
...
`experimental/` — Iterative model versions under active development. Do not import from these in pipeline without explicit integration.
...
`legacy/` — Original standalone model implementations, superseded by pipeline.
...
All pipeline behavior is controlled by YAML files in `pipeline/configs/`:
...
- **Type system** — shared types in `pipeline/utils/types.py`
- **Shared imports** — `pipeline/` is added to `sys.path` at the top of each runner script
...
cd pipeline && python scripts/validate_environment.py

# Run smoke tests (no audio files needed)
cd pipeline && python scripts/smoke_test.py

# Full pipeline CLI
cd pipeline && python inference/run_pipeline.py \

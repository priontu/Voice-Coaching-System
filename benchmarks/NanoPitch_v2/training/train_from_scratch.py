"""Train NanoPitch from scratch on GTSinger/English.

Initialises a fresh NanoPitch model (random weights) and trains it end-to-end
on the GTSinger English dataset using the voiced-weighted BCE loss identical
to finetune.py — all Conv1d, GRU, and output-head parameters are updated from
the first step.

Usage (run from the NanoPitch_v2 root):
    python training/train_from_scratch.py \\
        --gtsinger-root "/mnt/.../GTSinger/English" \\
        --output-dir ./runs/train_scratch_gtsinger

Resume from a checkpoint:
    python training/train_from_scratch.py ... --resume ./runs/train_scratch_gtsinger/checkpoints/last.pth
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Train NanoPitch from scratch on GTSinger")

    # Data
    p.add_argument("--gtsinger-root", required=True,
                   help="Path to GTSinger/English directory")
    p.add_argument("--output-dir", default="./runs/train_scratch_gtsinger")
    ...
    # Training schedule — higher lr and more warmup than fine-tuning
    p.add_argument("--epochs",        type=int,   default=100)
    p.add_argument("--lr",            type=float, default=1e-3,
                   help="Peak learning rate (higher than fine-tuning 1e-4)")
...
def load_checkpoint(model: NanoPitch, path: Path) -> tuple[int, float]:
    """Load weights and training state from a checkpoint. Returns (epoch, best_rpa)."""
    ...
    best_rpa = ckpt.get("best_rpa", rpa)
    print(f"  Loaded: epoch={epoch}  loss={loss:.5f}  rpa={rpa:.4f}")
    return epoch, best_rpa


def save_checkpoint(model: NanoPitch, args: argparse.Namespace,
                    epoch: int, loss: float, rpa: float,
                    best_rpa: float, path: Path) -> None:
    torch.save({
        "epoch":        epoch,
...
def build_pitch_targets(f0_batch: torch.Tensor, T: int,
    ...
    target  = target * voiced.unsqueeze(-1).float()
    return target
...
def main() -> None:
    ...
    if args.resume:
        ...
        resumed_epoch, best_rpa = load_checkpoint(model, Path(args.resume))
        start_epoch = resumed_epoch + 1
    else:
        n_params = sum(p.numel() for p in model.parameters())
        print(f"\nTraining from scratch: {n_params:,} parameters (all trainable)")
        ...
        for epoch in range(start_epoch, start_epoch + args.epochs):
            ...
            if epoch % args.save_every == 0:
                save_checkpoint(model, args, epoch, train_loss, best_rpa,
                                best_rpa, ckpt_dir / f"epoch_{epoch:03d}.pth")

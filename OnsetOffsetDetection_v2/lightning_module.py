...
try:
    ...
    raise RuntimeError("lightning is required for OnsetOffsetDetection_v2") from exc
...
from .model import CQTModelConfig, OnsetOffsetConformerCQT
from .preprocessing import CQTBatchProcessor, CQTFeatureConfig


@dataclass
class OnsetOffsetTrainingConfig:
    model: CQTModelConfig = field(default_factory=CQTModelConfig)
    features: CQTFeatureConfig = field(default_factory=CQTFeatureConfig)
...
class OnsetOffsetLightningModule(L.LightningModule):
    def __init__(self, config: OnsetOffsetTrainingConfig = OnsetOffsetTrainingConfig()):
        ...
        self.model = OnsetOffsetConformerCQT(config.model)
    ...
    def _process_raw_batch(self, raw_batch: dict[str, Any]) -> dict[str, Any]:
        ...
        if key not in self.processors:
            ...
            self.processors[key] = CQTBatchProcessor(

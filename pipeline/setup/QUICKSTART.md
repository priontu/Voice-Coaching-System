...
13. [Frontend — Web UI](#13-frontend--web-ui)
...
├── data/test/               # Sample test files (WAV, MusicXML, TextGrid)
...
The onset/offset detector uses a wav2vec2-based model saved as a PyTorch Lightning
checkpoint. Place it at:
...
This path is already the default in `configs/pipeline.yaml`. To disable onset/offset
detection entirely:
...
To point to a different checkpoint file:

```yaml
pipeline:
  enable_onset_offset: true
  checkpoints:
    onset_offset: "checkpoints/your_checkpoint.ckpt"
```

> `.ckpt` files (PyTorch Lightning) are loaded automatically via the wav2vec2 adapter.
> `.pt` files (plain PyTorch) load the legacy CNN+BiLSTM model instead.
...
The frontend is a NiceGUI web application that wraps the unified inference pipeline in a
browser-based interface. It accepts a WAV file upload, runs the full pipeline (pitch +
phoneme + onset/offset + scoring), and displays all four category scores with component
breakdowns and coach feedback.

### 13.1 Install NiceGUI

NiceGUI is included in `requirements.txt`. If you haven't installed it yet:
...
### 13.2 Start the frontend

Run from the `VocalCoach/` project root:
...
```
NiceGUI ready to go on http://localhost:8080, and http://...
```

Open your browser at: **http://localhost:8080**

> The server stays running until you press `Ctrl+C` in the terminal.

### 13.3 Using the frontend

#### Step 1 — Select a song

The **Song Setup** card at the top of the page shows a song selector. Each song in the
registry is linked to a MusicXML reference score and a Praat TextGrid file, which are used
for reference alignment and scoring.

The default song "Test Song" uses the files in `data/test/`:
- `data/test/test.musicxml`
- `data/test/test.TextGrid`

#### Step 2 — Choose input method

Click **Upload Audio File**. A large dashed upload area appears.

> "Sing Directly" is a placeholder and not yet functional.

#### Step 3 — Upload your recording

Drag and drop a WAV, MP3, M4A, or FLAC file onto the upload box, or click to browse.
After the upload finishes, the filename appears below the box.

```
Uploaded: my_recording.wav
Saved path: frontend\uploads\my_recording.wav
Ready. Click Analyze Singing.
```

#### Step 4 — Analyze

Click **Analyze Singing**. The status label changes to:

```
Running unified pipeline... (this may take a minute)
```

The pipeline runs in the background (model loading + inference + scoring). GPU inference
typically takes 10–30 seconds for a 10-second recording; CPU inference takes longer.

#### Step 5 — Read your results

When analysis completes, the results page appears with:

| Section | What it shows |
|---------|---------------|
| **Score cards** | Five cards: Overall, Pitch, Timing, Duration, Lyrics — each with a 0–100 score and a level label (Excellent / Good / Fair / Needs Work) |
| **Pitch detail** | Component progress bars: Pitch Accuracy, Intonation (MACE), Stability — with raw metric values |
| **Timing detail** | Onset Accuracy, Mean Onset Error, Rhythm Stability |
| **Duration detail** | Duration Accuracy, Duration Ratio, Phrase Duration |
| **Lyrics detail** | Word Alignment, Phoneme Overlap, Label Match, Phoneme Timing |
| **Coach Feedback** | Strengths (green) and Areas to Improve (orange) from the interpretation engine |
| **Debug Output** | Collapsible panel showing raw pipeline stdout/stderr |

#### Removing an uploaded file

Click **✕ Remove File** to delete the current upload and reset the form before uploading
a different recording.

#### Toggling the theme

Click **Toggle Theme** in the top-right corner to switch between dark and light mode.

### 13.4 Add a new song

Edit `frontend/app.py` and add an entry to `SONG_REGISTRY`:

```python
SONG_REGISTRY = {
    "Test Song": {
        "musicxml": PROJECT_DIR / "data/test/test.musicxml",
        "textgrid": PROJECT_DIR / "data/test/test.TextGrid",
    },
    "My Song": {
        "musicxml": PROJECT_DIR / "references/my_song.musicxml",
        "textgrid": PROJECT_DIR / "references/my_song.TextGrid",
    },
}
```

Both `musicxml` and `textgrid` are required for full scoring. Restart the server after
saving.

### 13.5 Scores show "N/A"

Scores are `None` (displayed as "N/A") when:

- The reference files are missing or could not be parsed
- The pipeline ran without `--compute-scores` (this is always enabled in the frontend)
- No note matches were found during alignment (check `alignment_tolerance_ms` in
  `configs/pipeline.yaml` — the default is 100 ms)

Check the **Debug Output** section in the results page for the full pipeline log.

### 13.6 Troubleshooting the frontend

| Problem | Fix |
|---------|-----|
| `ModuleNotFoundError: nicegui` | `pip install nicegui>=1.4.0` |
| Port 8080 already in use | `py frontend\app.py` — NiceGUI will auto-select another port; watch the console |
| Upload fails silently | Check that `frontend/uploads/` is writable |
| Pipeline fails (all scores N/A) | Expand **Debug Output** in the UI; check that `data/test/test.musicxml` and `data/test/test.TextGrid` exist |
| Slow analysis on CPU | See §12.12 — disable unused modules or use GPU |

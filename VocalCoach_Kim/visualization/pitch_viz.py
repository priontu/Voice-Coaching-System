"""
visualization/pitch_viz.py - Plotting utilities for the pitch + VAD pipeline.

Moved from the original Pitch Model w VAD/visualization.py.
All plotting functions are unchanged — only the module path has changed.
"""
...
def _shade_voiced_regions(
    ...
    for v, t in zip(voiced_mask, timestamps):
        if v and not in_seg:
            seg_start = t - half
            in_seg = True
        elif not v and in_seg:
            ax.axvspan(seg_start, t - half, color=color, alpha=alpha,
                       label=label if first else "_nolegend_")
            in_seg = False
            first = False

    if in_seg:
        ax.axvspan(seg_start, float(timestamps[-1]) + half, color=color, alpha=alpha,
                   label=label if first else "_nolegend_")
...
def plot_waveform(
    ...
    ax.set_xlim(times[0], times[-1]); ax.grid(True, alpha=0.3)
    fig.tight_layout()
...
def plot_pitch_contour(
    ...
    if reference_f0 is not None:
        ...
        ax.plot(times[ref_voiced], reference_f0[ref_voiced],
...
def plot_pitch_vad_combined(
    ...
    audio_times = np.arange(len(audio)) / sr
    use_vad = vad_times is not None and vad_mask is not None
    n_panels = 3 if use_vad else 2
    heights = [2, 1, 3] if use_vad else [2, 3]

    fig, axes = plt.subplots(n_panels, 1, figsize=(14, 2 * n_panels + 2),
                             gridspec_kw={"height_ratios": heights}, sharex=False)
    ...
    t_min, t_max = float(audio_times[0]), float(audio_times[-1])

    ax_wave.plot(audio_times, audio, color="steelblue", linewidth=0.5, alpha=0.9)
    _shade_voiced_regions(ax_wave, voiced_mask, pitch_times, color="green", alpha=0.2, label="Voiced (fused)")
    ax_wave.set_ylabel("Amplitude"); ax_wave.set_xlim(t_min, t_max)
    ax_wave.legend(loc="upper right", fontsize=7); ax_wave.grid(True, alpha=0.25)
    ax_wave.set_title(title)

    if ax_vad is not None and use_vad:
        ax_vad.fill_between(vad_times, vad_mask.astype(float), step="mid",
                            color="green", alpha=0.7, label="VAD voiced")
        ax_vad.set_yticks([0, 1]); ax_vad.set_yticklabels(["Unvoiced", "Voiced"], fontsize=7)
        ax_vad.set_xlim(t_min, t_max); ax_vad.set_ylim(-0.05, 1.15)
        ax_vad.set_ylabel("VAD"); ax_vad.legend(loc="upper right", fontsize=7)
    ...
    if reference_f0 is not None:
        ref_voiced = reference_f0 > 0
        ax_pitch.plot(pitch_times[ref_voiced], reference_f0[ref_voiced],
                      color="darkorange", linewidth=1.5, alpha=0.8, label="Reference F0", zorder=2)

    ax_pitch.set_xlabel("Time (s)"); ax_pitch.set_ylabel("F0 (Hz)")
    ax_pitch.set_xlim(t_min, t_max)
    ax_pitch.legend(loc="upper right", fontsize=7); ax_pitch.grid(True, alpha=0.25)
...
def plot_cent_error(
    ...
    ax.axhline(threshold_cents, color="red", linestyle="--", linewidth=1,
               label=f"+{threshold_cents:.0f} cents")
    ax.axhline(-threshold_cents, color="red", linestyle="--", linewidth=1,
               label=f"-{threshold_cents:.0f} cents")
    ax.axhline(0, color="black", linestyle="-", linewidth=0.8, alpha=0.5)
    ax.set_title(title); ax.set_xlabel("Time (s)"); ax.set_ylabel("Error (cents)")
    ax.legend(loc="upper right", fontsize=8); ax.grid(True, alpha=0.3)
    fig.tight_layout()
    return _save_or_return(fig, save_path)
...
def plot_note_level_errors(
    ...
    ax.axhline(threshold_cents, linestyle="--", color="gray", linewidth=1)
    ax.axhline(-threshold_cents, linestyle="--", color="gray", linewidth=1)
    ...
    ax.set_title(title); ax.set_xlabel("Note Index"); ax.set_ylabel("Median Cent Error")

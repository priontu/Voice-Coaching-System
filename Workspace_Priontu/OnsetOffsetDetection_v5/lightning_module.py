...
try:
    ...
    raise RuntimeError("lightning is required for OnsetOffsetDetection_v5") from exc

from OnsetOffsetDetection_v1.lightning_module import masked_bce_with_logits, masked_mean
from OnsetOffsetDetection_v1.preprocessing import OnsetOffsetLabelConfig

from .model import Wav2Vec2ModelConfig, Wav2Vec2OnsetDetector
from .preprocessing import Wav2Vec2BatchProcessor, Wav2Vec2FeatureConfig
...
class OnsetOffsetTrainingConfig:
    model: Wav2Vec2ModelConfig = field(default_factory=Wav2Vec2ModelConfig)
    features: Wav2Vec2FeatureConfig = field(default_factory=Wav2Vec2FeatureConfig)
    labels: OnsetOffsetLabelConfig = field(default_factory=OnsetOffsetLabelConfig)
    learning_rate: float = 1e-4     # lower LR for fine-tuning a pretrained model
...
class OnsetOffsetLightningModule(L.LightningModule):
    def __init__(self, config: OnsetOffsetTrainingConfig = OnsetOffsetTrainingConfig()):
        ...
        self.model = Wav2Vec2OnsetDetector(config.model)
    ...
    def _process_raw_batch(self, raw_batch: dict[str, Any]) -> dict[str, Any]:
        ...
        if key not in self.processors:
            ...
            self.processors[key] = Wav2Vec2BatchProcessor(
    ...
    def _loss(self, outputs: dict[str, torch.Tensor], batch: dict[str, Any]) -> tuple[torch.Tensor, dict]:
        # Use frame_attention_mask (frame-level) for loss, not the sample-level attention_mask
        mask = batch["frame_attention_mask"].float()
        labels = batch["labels"]

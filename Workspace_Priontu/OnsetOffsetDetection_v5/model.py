...
class Wav2Vec2ModelConfig:
    ...
    freeze_feature_extractor: bool = True   # freeze CNN feature extractor, fine-tune transformer
    freeze_transformer: bool = False        # set True for a linear-probe baseline
    dropout: float = 0.1
...
class Wav2Vec2OnsetDetector(nn.Module):
    ...
    layers are fine-tuned. Raw 16 kHz waveform goes in, per-frame logits come out
    at ~50 Hz (20 ms / frame).
    """
    ...
    def __init__(self, config: Wav2Vec2ModelConfig = Wav2Vec2ModelConfig()):
        ...
        try:
            ...
            raise RuntimeError(
                "transformers is required for OnsetOffsetDetection_v5. "
                "Install with: pip install transformers"
            ) from exc

        self.config_obj = config
        self.wav2vec2 = Wav2Vec2Model.from_pretrained(config.pretrained_model_name, use_safetensors=True)

...
try:
    ...
    raise RuntimeError("lightning is required for OnsetOffsetDetection_v4") from exc
...
from .model import MambaModelConfig, OnsetOffsetMamba
...
class OnsetOffsetTrainingConfig:
    ...
    max_audio_sec: float | None = None  # Mamba is O(n) — no cap needed
...
class OnsetOffsetLightningModule(L.LightningModule):
    def __init__(self, config: OnsetOffsetTrainingConfig = OnsetOffsetTrainingConfig()):
        ...
        self.model = OnsetOffsetMamba(config.model)
